...
async function run() {
  ...
  console.log('=== v0.6.0 Verification ===\n');
  ...
  // --- 1. Welcome screen renders on page load (before any JS action) ---
  ...
  // --- 2. KEY STRUCTURAL CHECK: Welcome content in HTML source (not JS-generated) ---
  // The raw HTML source should contain the welcome heading before any JS executes
  const htmlSource = await page.content();
  assert(
    htmlSource.includes('class="welcome"') || htmlSource.includes("class='welcome'"),
    'Welcome .welcome element is in the HTML source'
  );
  assert(
    htmlSource.includes('Sparrow Deck'),
    'Heading "Sparrow Deck" is in the HTML source'
  );
  assert(
    htmlSource.includes('Learn guild names'),
    'Button "Learn guild names" is in the HTML source'
  );
  assert(
    htmlSource.includes('say it out loud') || htmlSource.includes('Say it out loud'),
    'Subtext "say it out loud" is in the HTML source'
  );

  // --- 3. Version footer shows v0.6.0 ---
  const footerText = await page.textContent('#app-version');
  assert(footerText && footerText.includes('v0.6.0'), `Footer shows v0.6.0 (got "${footerText}")`);

  // --- 4. All welcome content elements present ---
  const heading = await page.$('.welcome-heading');
  assert(heading !== null, '.welcome-heading element present');
  const headingText = await heading.textContent();
  assert(headingText === 'Sparrow Deck', `Heading is "Sparrow Deck" (got "${headingText}")`);
  ...
  assert(
    ...
    `Instructions mention "Boros"`
  );
  assert(
    instructionsText.toLowerCase().includes('name'),
    `Instructions mention guessing a name`
  );
  ...
  assert(
    ...
    `Subtext contains "say it out loud"`
  );
  ...
  // --- 5. Spacebar does NOT advance cards while on welcome screen ---
  ...
  // --- 6. Clicking the app background does NOT start session ---
  ...
  // --- 7. Clicking "Learn guild names" starts the card session ---
  ...
  // --- 8. Progress counter starts at Card 1 ---
  ...
  // --- 9. Pause and Stop buttons visible ---
  ...
  // --- 10. Cards cycle normally (auto-reveal and auto-advance) ---
  ...
  // --- 11. Footer has trace link after session starts ---
  ...
  // --- 12. Stop session and check session end screen ---
  ...
  await page.screenshot({ path: 'scripts/v0.6.0-screenshot-end.png', fullPage: true });
  console.log('\nScreenshot saved to scripts/v0.6.0-screenshot-end.png');

  // --- 14. Reload and verify welcome screen appears again (static HTML behavior) ---
  ...
  // Verify the reloaded HTML source also has static welcome content
  const reloadedSource = await page.content();
  assert(
    reloadedSource.includes('Sparrow Deck') && reloadedSource.includes('Learn guild names'),
    'Welcome content still in HTML source after reload'
  );

  // --- 15. Wait for Honeycomb spans to flush ---

...
## DEC-203: Per-Combo og:Images Generated as Separate Build Step
- **Date**: 2026-03-26
- **Decision**: The 20 per-combo `og:image` PNGs are generated by a standalone script (separate from the HTML combo build) and stored in `images/combo/`. They are checked into git alongside the generated HTML pages.
- **Context**: Arc 51 adds unique social share images for each of the 20 combo pages. Images depict the pentagon with highlighted combo colors, combo name, and tier, in the Arc 50 visual style. The HTML combo pages are regenerated on each build (descriptions, card lists can change); images change only when the visual design changes.
- **Rationale**: Images change less often than page content. Keeping the image generation separate keeps normal builds fast. 20 PNGs (~4–8MB total) are acceptable for a static site hosted on GitHub Pages.
- **Alternative considered**: Generating images at deploy time as part of the standard build — rejected because it adds Playwright as a build dependency and increases build time for changes that don't affect image appearance.

## DEC-204: Discoverability Files Are Static, Not Generated
- **Date**: 2026-03-26
- **Decision**: `robots.txt`, `sitemap.xml`, and `llms.txt` are hand-authored static files, not generated by a build script.
